...
contract BondingMathematics {
    ...
    */
    ...
    /*
    * @dev function calcPurchase returns Mogul tokens amount
    *
    * @param continuousTokenSupply uint256 continuous Token total supply
    * @param premintedMGL uint256 preminted Mogul supply
    * @param daiAmount uint256 usd buying amount
    */
    function calcPurchase(uint256 continuousTokenSupply,
        uint256 premintedMGL,
        uint256 usdAmount) public view returns (uint256){

        (bool success, bytes memory data) = vyperMath.staticcall(abi.encodeWithSignature("calc_purchase(uint256,uint256,uint256)", continuousTokenSupply, premintedMGL, usdAmount));
        require(success);
